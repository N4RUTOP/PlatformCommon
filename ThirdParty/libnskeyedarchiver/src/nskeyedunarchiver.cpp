...
NSKeyedUnarchiver::NSKeyedUnarchiver(NSClassManager* class_manager, const char* data, size_t length)
  ...
  plist_from_bin(data, (uint32_t)length, &plist);
  if (plist == nullptr) {
...
plist_t NSKeyedUnarchiver::DereferenceObject(plist_t object_ref) const {
  ...
  if (0 <= uid && uid < objects_size_) {
    return plist_array_get_item(objects_, (uint32_t)uid);
  }
  return nullptr;
}
...
int NSKeyedUnarchiver::CurrentDecodingContextDepth() const { return (int)containers_.size(); }
